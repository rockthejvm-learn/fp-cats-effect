...
object Resources extends IOApp.Simple {
  ...
  /** Exercise: read a text file with one line every 100 millis, using Resource (refactor the bracket exercise
    * to use Resource)
    */
  ...
  val openConnection = connFromConfResourceClean("cats-effect/src/main/resources/connection.txt").use(conn =>
    conn.open() >> IO.never
  )
  ...
  override def run: IO[Unit] =
    bracketReadFile("src/main/scala/com/udavpit/fp/cats/effect/util/Utils.scala")
}
